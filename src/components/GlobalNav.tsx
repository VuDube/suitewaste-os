    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-background/90 backdrop-blur-2xl border-t border-white/5 z-50 flex items-center justify-around px-2 pb-[env(safe-area-inset-bottom)]">
      ...
      <Sheet>
        <SheetTrigger asChild>
          <button 
            className="flex flex-col items-center justify-center w-16 h-14 rounded-2xl text-muted-foreground active:scale-90 transition-transform" 
            onClick={triggerHaptic}
          >
            <Menu className="w-5 h-5 mb-1" />
            <span className="text-[8px] font-black uppercase tracking-widest">App Hub</span>
          </button>
        </SheetTrigger>
        <SheetContent side="bottom" className="bg-background/95 backdrop-blur-3xl rounded-t-[3rem] p-8 border-t border-white/10 max-h-[85vh] overflow-y-auto scrollbar-hide">
          <SheetHeader className="mb-8">
            <SheetTitle className="text-3xl font-black uppercase tracking-tighter">Master-Suite</SheetTitle>
            <SheetDescription className="text-muted-foreground font-bold uppercase text-[10px] tracking-widest flex items-center gap-2">
              <Zap className="h-3 w-3 text-primary" /> Enterprise Ecosystem Active
            </SheetDescription>
          </SheetHeader>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {group.items.filter(i => !userRole || i.roles.includes(userRole)).map(item => (
                    <NavLink 
                      ...
                      to={item.href} 
                      onClick={triggerHaptic} 
                      className="flex flex-col items-center gap-2 p-4 rounded-3xl bg-surface-variant/40 active:bg-primary/20 active:scale-95 transition-all group"
                    >
                      <item.icon className="h-6 w-6 text-foreground/80 group-active:text-primary" />
          ...
          <Button 
            ...
            variant="destructive" 
            className="w-full h-16 rounded-[2rem] font-black uppercase tracking-widest text-base shadow-2xl active:scale-95 transition-transform mb-4"
          >
            <LogOut className="mr-2 h-5 w-5" /> End Session
          </Button>
        </SheetContent>
      </Sheet>
    </div>
